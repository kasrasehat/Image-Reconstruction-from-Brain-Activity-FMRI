...
fmri_dim = int(np.ceil(np.sqrt(torch.max(torch.tensor([next(iter(dataloader1))[1].shape[1], next(iter(dataloader2))[1].shape[1], next(iter(dataloader3))[1].shape[1]])))))**2
gen = unet(z_dim=fmri_dim).to(device)

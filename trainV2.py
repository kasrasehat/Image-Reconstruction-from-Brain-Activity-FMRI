...
from comparator import CBIR

torch.manual_seed(0) # Set for testing purposes, please do not change!
from Networks import Generator1, Critic
...
gen = Generator1(fmri_dim).to(device)
gen.feature_extractor_BEIT.requires_grad_(False)
gen_opt = torch.optim.Adam(gen.parameters(), lr=lr1, betas=(beta_1, beta_2))
crit = Critic().to(device)
crit_opt = torch.optim.Adam(crit.parameters(), lr=lr2, betas=(beta_1, beta_2))
def weights_init(m):
    if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
...
for epoch in tqdm(range(n_epochs)):
    ...
    for i, data in enumerate(zip(dataloader_real,dataloader1,dataloader2)):
        ...
        for subject in [data[1], data[2]]:
            ...
            cbir = CBIR()
            sub_image_feature = cbir.get_feature(batch_of_image=sub_images)
            critic_losses += [mean_iteration_critic_loss]
            ### Update generator ###
            gen_opt.zero_grad()
            fake_2, feature = gen(fake_noise)
